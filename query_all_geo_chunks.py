...
num_tweets = meta_data.count()


slices = arange(0, num_tweets, int(chunk_size))
slices = append(slices, num_tweets-1)
for i in range(85,len(slices)-1):
	meta_data = tweets_collection.find(query, projection)[slices[i]:slices[i+1]]
	print i


	meta = [m for m in meta_data]
	data_file = [['display_url', 'media_url', 'size_h', 'size_w', 'lat', 'lon', 'favoritesCount', 'followersCount', 'friendsCount', 'statusesCount', 'profile_url', 'post_url', 'img_url', 'postedTime', 'location', 'country', 'mongo_id']]
	...
	for ind in range(len(meta)):
		...
		mongo_id = str(meta[ind]['_id'])
		data_file.append([display_url, media_url, size_h, size_w, lat, lon, favoritesCount, followersCount, friendsCount, statusesCount, profile_url, post_url, img_url, postedTime, location, country, mongo_id])
	...
	with open("./2013/output_2013_chunk_" + str(i)+ ".csv", "wb") as f:
		writer = csv.writer(f)
